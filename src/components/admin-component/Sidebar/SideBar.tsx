...
import { ArrowLeftFromLine, ArrowRightFromLine, ChartPie } from "lucide-react";
...
export default function SideBar() {
  const listSideBar: any = sideBarAdmin;
  ...
  return (
    <div className={`h-screen px-3 py-4 overflow-y-auto bg-neutral-950 text-stone-100 ${showSideBar ? 'w-[220px]' : 'w-[70px]'}`}>
      ...
      <div className="pl-2 mb-4">
        <button onClick={e => setShowSideBar(!showSideBar)} className="border p-2 rounded text-stone-400 border-stone-400 hover:border-stone-100 hover:text-stone-100">
          {showSideBar ? <ArrowLeftFromLine size={16} /> : <ArrowRightFromLine size={16} />}
        </button>
      </div>
      {listSideBar &&
        listSideBar.map((item: any, index: number) => (
          <div key={index} className="mb-2 border-b border-stone-700">
            ...
            {item.listItems.map((childItem: any, childIndex: number) => (
              <ul key={childIndex} className="space-y-2 font-medium">
                <li className="relative" onMouseEnter={() => setHoveredItem(childItem.name)} onMouseLeave={() => setHoveredItem(null)}>
